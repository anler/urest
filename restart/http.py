# coding: utf-8
"""The various HTTP responses used for returning proper HTTP codes to clients."""
...
http_method_names = set(["get", "post", "put", "delete", "head", "options", "trace"])
...
class Ok(Response):
    ...
    Should be used to indicate nonspecific success. Must not be used to communicate errors in the
    response body. In most cases, 200 is the code the client hopes to see. It indicates that
    the REST API successfully carried out whatever action the client requested, and that no more
    specific code in the 2xx series is appropriate. Unlike the 204 status code, a 200 response
...
class Created(Response):
    ...
    A REST API responds with the 201 status code whenever a collection creates, or a store adds, a
    new resource at the client's request. There may also be times when a new resource is created
    as a result of some controller action, in which case 201 would also be an appropriate response.
    """
    status_code = 201
...
class Accepted(Response):
    ...
    problems once it's finally processed. A 202 response is typically used for actions that take a
    long while to process. Controller resources may send 202 responses, but other resource types
    should not.
    """
    status_code = 202
...
class NoContent(Response):
    ...
    REST API declines to send back any status message or representation in the response message's
    body. An API may also send 204 in conjunction with a GET request to indicate that the requested
...
class MultipleChoices(Response):
    ...
    It could be used to present different format options for video, list files with different
...
class MovedPermanently(Response):
    ...
    redesigned and a new permanent URI has been assigned to the client's requested resource. The
    REST API should specify the new URI in the response's Location header.
    """
    status_code = 301
...
class Found(Response):
    ...
    Should not be used.
    The intended semantics of the 302 response code have been misunderstood by programmers and
    incorrectly implemented in programs since version 1.0 of the HTTP protocol. The confusion
    centers on whether it is appropriate for a client to always automatically issue a follow-up GET
    request to the URI in response's Location header, regardless of the original request's
    method. For the record, the intent of 302 is that this automatic redirect behavior only applies
    if the client's original request used either the GET or HEAD method.
    To clear things up, HTTP 1.1 introduced status codes 303 ("See Other") and 307 ("Temporary
    Redirect"), either of which should be used instead of 302.
    """
    status_code = 302
...
class NotModified(Response):
    ...
    This status code is similar to 204 ("No Content") in that the response body must be empty. The
    key distinction is that 204 is used when there is nothing to send in the body, whereas 304 is
    used when there is state information associated with a resource but the client already has the
    most recent version of the representation.
    """
    status_code = 304
...
class TemporaryRedirect(Response):
    ...
    HTTP/1.1 introduced the 307 status code to reiterate the originally intended semantics of the
    302 ("Found") status code. A 307 response indicates that the REST API is not going to process
    the client's request.  Instead, the client should resubmit the request to the URI specified by
    ...
    resource. For example, a 307 response can be used to shift a client request over to another
    host.
    """
    status_code = 307
...
class Forbidden(Response):
    ...
    would be 401 ("Unauthorized").  REST APIs use 403 to enforce application-level permissions. For
    example, a client may be authorized to interact with some, but not all of a REST API's
    resources. If the client attempts a resource interaction that is outside of its permitted
...
class MethodNotAllowed(Response):
    ...
    the resource does not allow. For instance, a read-only resource could support only GET and
    HEAD, while a controller resource might allow GET and POST, but not PUT or DELETE.
...
class NotAcceptable(Response):
    ...
    preferred media types, as indicated by the Accept request header. For example, a client request
    for data formatted as application/xml will receive a 406 response if the API is only willing to
...
class Conflict(Response):
    ...
    The 409 error response tells the client that they tried to put the REST API's resources into an
    impossible or inconsistent state. For example, a REST API may return this response code when a
    client tries to delete a non-empty store resource.
    """
    status_code = 409
...
class UnsupportedMediaType(Response):
    ...
    Must be used when the media type of a request's payload cannot be processed.  The 415 error
    response indicates that the API is not able to process the client's supplied media type, as
    indicated by the Content-Type request header.
    For example, a client request including data formatted as application/xml will receive a 415
...
class NotImplemented(Response):
    ...
    The server either does not recognise the request method, or it lacks the ability to fulfill the
    request.
    """
    status_code = 501
...
def api_endpoint(view=None, accept=None, serializer=None, unserializer=None):
    def decorator(view):
        return api_endpoint_property(view, accept=accept, serializer=serializer,
...
class api_endpoint_property(object):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if content_type not in self._accept:
...
class ResponseDescriptor(object):
    ...
    def __get__(self, request, request_class=None):
        return functools.partial(get_response, content_type=request.META["CONTENT_TYPE"],
